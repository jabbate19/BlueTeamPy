...
def main():
    ...
    for line in lines:
        ...
        if target_check:
            ...
            input()
    print("===CRON===")
    for cron_location in ["anacrontab", "cron.d", "cron.daily",
    ...
    for user in UserInfo.get_all_users():
        for shell_config in [".*rc", ".*profile",".*history",
                             ".*env",".*login", ".*logout",
                             ".*aliases", ".*bindings"]:
            ...
            if not user_find[2]:
                for result in str(user_find[0], "utf8").split("\n"):
                    ...
                    if not user_grep[2]:
                        ...
                        input()
    for file_location in ["/etc/profile", "/etc/bashrc", "/etc/bash.bashrc",
                          "/etc/bash_completion", "/etc/bash_completion.d",
                          "/etc/zsh", "/etc/profile.d", "/etc/inputrc"]:
